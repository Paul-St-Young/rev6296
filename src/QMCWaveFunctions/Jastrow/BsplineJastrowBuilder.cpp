  ...
  else // Create a two-body Jastrow
  {
    ...
    while (kids != NULL)
    {
      ...
      if (kidsname == "correlation")
      {
        ...
        string link_coeff_name("none"); // link Jastrow coefficients
        ...
        } else { // look for linked coefficients
    ...
    success = dJ2->checkInitialization();
    if (!success) APP_ABORT("failed to initialize two-body Jastrow derivatives");
